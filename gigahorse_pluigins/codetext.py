...
def load_csv_specific(path: str, seperator: str='\t', first: int = 0, second: int = 1) -> Mapping[str, str]:
    with open(path) as f:
        return {line.split(seperator)[first]: line.split(seperator)[second] for line in f.read().splitlines()}

def load_target_slot_csv_map(path: str, separator: str = '\t') -> Dict[str, Tuple[str, int, int]]:
    stmt_to_storage = {}
...
def emit_call(stmt: Statement, out: TextIO):
    if stmt.operands[1] in variable2value:  # known const target
        if stmt.ident in stmt2resolvedExternalCall:  # resolved
            emit(f'{variable2value[stmt.operands[1]]}.{stmt2resolvedExternalCall[stmt.ident]}', out, 2)
        else:
        ...
        if stmt.ident in stmt2resolvedExternalCall:  # resolved
            emit(f'{stmt2targetAddress[stmt.ident]}.{stmt2resolvedExternalCall[stmt.ident]}', out, 2)
        else:
        ...
        if stmt.ident in stmt2resolvedExternalCall: # resolved
            ...
            emit("UnknownTarget.UnknownExternalCall")
...
def construct_pruned_cfg() -> Tuple[Mapping[str, Block], Mapping[str, Function]]:
    ...
    for block_id in chain(*function2blocks.values()):
        try:
            statements = [
                ...
                if stmt2op[s_id] in {"CALL", "STATICCALL", "DELEGATECALL", "CALLPRIVATE"}
            ]
    ...
    for block_id, in load_csv(get_full_path('IRFunctionEntry.csv')):
        ...
        functions[func_id] = Function(func_id, high_level_name, blocks[block_id], func_id in publicFunc2selector or func_id == '0x0')
    return blocks, functions

def print_codetext(out):
    emit("CONTRACT START", out)
    filtered_functions = [f for f in functions.values() if f.name.split('(')[0] != '__function_selector__']
    public_functions = sorted([f for f in filtered_functions if f.is_public], key=lambda x: x.ident)
    private_functions = sorted([f for f in filtered_functions if not f.is_public], key=lambda x: x.ident)
...
def get_storage_address(stmt2ExternalCallTargetStorage, address, chain, block_number):
    ...
    for stmt_id, (slot_index, byteLow, byteHigh) in stmt2ExternalCallTargetStorage.items():
...
def main():
    ...
    stmt2resolvedExternalCall = load_csv_specific(get_full_path('ExternalCallResolved.csv'),'\t',1,3)
    stmt2resolvedExternalCall = {stmt: func.split('(')[0] for stmt, func in stmt2resolvedExternalCall.items()}
    stmt2ExternalCallTargetStorage = load_target_slot_csv_map(get_full_path("ExternalCallTargetStorage.csv"))
    global stmt2targetAddress
    if len(stmt2ExternalCallTargetStorage) != 0:
        stmt2targetAddress = get_storage_address(stmt2ExternalCallTargetStorage, '0x9c63d6328C8e989c99b8e01DE6825e998778B103', 'bsc', 37914141)
    
    
    global functions
    _, functions,  = construct_pruned_cfg()
    

    with open('codetext.tac', 'w') as f:
        print_codetext(f)


w3bsc =  Web3(Web3.HTTPProvider("https://fluent-late-replica.bsc.quiknode.pro/bf8524872ba95e695f9e060b9d472fbf3cacdf1f"))
w3eth = Web3(Web3.HTTPProvider('https://eth-mainnet.g.alchemy.com/v2/A2dA3ZH-_jrg1FRt8ZAdpvs1QWGpoRHF'))
